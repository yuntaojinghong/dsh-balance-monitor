...
- **总开关**：配置页主开关或卡片「停用」按钮可一键停用全部监控（不再查询余额、不拦截任务、不弹提醒），接入其他模型服务商时不再打扰
...
- **停用**：点击卡片上的「停用」按钮一键关闭全部监控（接入其他模型服务商时使用）；停用后卡片变为「余额监控已停用 · 点此启用」，点击即恢复

### 配置页（设置 → 插件 → 余额监控）

| 配置项 | 说明 | 默认值 |
| --- | --- | --- |
| 启用余额监控（总开关） | 一键停用/启用全部监控行为；接入其他模型服务商时建议关闭 | `开启` |
| 余额阈值（元） | 低于该值视为「不足」，触发暂停询问与通知 | `10` |
| 自动检查间隔（秒） | 后台查询余额的频率，修改后立即生效（最大 86400） | `60` |
| 强制检查间隔（分钟） | 任务步骤前自动检查的节流间隔（最大 10080） | `5` |
...
| 字段 | 类型 | 含义 |
| --- | --- | --- |
| `enabled` | boolean | 总开关：`false` 时停止查询余额、强制检查与提醒 |
...
- **总开关（enabled）**：关闭后自动查询、强制检查与工具调用一律跳过，客户端隐藏余额卡片、停止通知；重新开启立即恢复。
...
**Q：接入其他模型服务商（非 DeepSeek）时还弹余额不足？**
A：余额监控只面向 DeepSeek 账户。使用其他服务商时，在 设置 → 插件 → 余额监控 关闭「启用余额监控」总开关（或点击卡片上的「停用」），插件即停止查询与提醒；需要时再开启。
