 ...
 *
 * __ModuleLoader__ 工厂形态（与 dsh-vision-router 一致）。
 * 通过 fetch('/balance-api/*') 调用宿主 webServer 路由（无 typert 依赖）。
...
`
    ...
    async function api(path, options) {
      const res = await fetch('/balance-api' + path, options)
      return res.json()
    }
    ...
    function apply(ctx) {
      ...
      function SettingsView() {
        ...
        useEffect(() => {
          ...
          }).catch((e) => { setError('无法读取状态：' + String(e && e.message || e)) })
          loadHistory()
        ...
        const checkNow = () => {
          setBusy(true)
          api('/check').then(() => { refreshState(); loadHistory() }).catch((e) => {
        ...
        const goRecharge = () => {
          api('/recharge-clicked', { method: 'POST' }).then(() => refreshState()).catch(() => {})
      ...
      function ChatCard() {
          ...
          api('/state').then((s) => {
            ...
            if (s !== null && typeof s === 'object' && typeof s.cardX === 'number' && typeof s.cardY === 'number') {
              ...
              if (c.x !== s.cardX || c.y !== s.cardY) {
                api('/set-config', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ cardX: c.x, cardY: c.y }) }).catch(() => {})
              }
        ...
        const toggleEnabled = (next) => {
          ...
          }).then((s) => { if (s !== null && typeof s === 'object') setSnap(s) }).catch(() => {})
        }
        ...
        const onPointerDown = (e) => {
          ...
          const up = (ev) => {
            ...
            setDragging(false)
            const c = clampToViewport(baseX + ev.clientX - startX, baseY + ev.clientY - startY, rect.width, rect.height)
            api('/set-config', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ cardX: Math.round(c.x), cardY: Math.round(c.y) }) }).catch(() => {})
          }
