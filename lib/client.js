/**
 * dsh-balance-monitor · 客户端插件（npm 包版）
 *
 * 形态：window.__ModuleLoader__.load({ id, factory }) —— 与 dsh-vision-router 等
 * 第三方客户端插件一致。工厂接收 require（CJS），React 通过 require('react') 获取，
 * 内核将 { name, inject, apply } 采纳为客户端插件。
 *
 * 与 client.js（动态插件版）功能一致，区别：
 *  - 调用宿主：ctx.remote.balance.<method>()（Host 侧 TypertRemoteService 暴露）
 *  - CSS：直接注入 <style data-plugin-css> 标签
 */
...
`
    function injectCss() {
      if (typeof document !== 'undefined' && document.querySelector('style[data-plugin-css="dsh-balance-monitor"]') === null) {
        const tag = document.createElement('style')
        tag.setAttribute('data-plugin-css', JSON.stringify('dsh-balance-monitor'))
        tag.textContent = CSS
        document.head.appendChild(tag)
      }
    }

    const name = 'balance-monitor-client'
    const inject = ['remote', 'remote.balance']
    ...
    function apply(ctx) {
      ...
      function SettingsView() {
        ...
        const [cardDraft, setCardDraft] = useState(true)
        const [notifyDraft, setNotifyDraft] = useState(true)
        const [autoSecDraft, setAutoSecDraft] = useState('60')
        const [forceMinDraft, setForceMinDraft] = useState('5')
        const [history, setHistory] = useState(null)
        const [busy, setBusy] = useState(false)
        const [error, setError] = useState(null)

        const refreshState = () => {
          ctx.remote.balance.state().then((s) => {
            setSnap(s)
            setError(null)
            setBusy(false)
          }).catch((e) => {
        ...
        const loadHistory = () => {
          ctx.remote.balance.history().then((h) => {
        ...
        const checkNow = () => {
          setBusy(true)
          ctx.remote.balance.check().then(() => { refreshState(); loadHistory() }).catch((e) => {
        ...
        const goRecharge = () => {
          ctx.remote.balance.rechargeClicked().then(() => refreshState()).catch(() => {})
      ...
      function ChatCard() {
          ...
          ctx.remote.balance.state().then((s) => {
            ...
            if (s !== null && typeof s === 'object' && typeof s.cardX === 'number' && typeof s.cardY === 'number') {
              ...
              if (c.x !== s.cardX || c.y !== s.cardY) {
                ctx.remote.balance.setConfig({ cardX: c.x, cardY: c.y }).catch(() => {})
              }
        ...
        const onPointerDown = (e) => {
          ...
          const up = (ev) => {
            ...
            setDragging(false)
            const c = clampToViewport(baseX + ev.clientX - startX, baseY + ev.clientY - startY, rect.width, rect.height)
            ctx.remote.balance.setConfig({ cardX: Math.round(c.x), cardY: Math.round(c.y) }).catch(() => {})
          }
